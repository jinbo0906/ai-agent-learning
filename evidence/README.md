...
```
evidence/ch01/
├── notes.md            学习笔记（已提交到公开仓库的内容可被阅读）
├── report.md           实验报告（数字必须能在 summary.json 定位到）
├── article.md          逐章分享文章草稿（完成后可另行发布）
...
2. **`summary.json` 是证据体系的锚点。** 报告和文章里出现的每个数字，
   ...
   当前 `attest check` 只检查结果文件是否存在，数字交叉核对尚未实现；提交前需人工核对。

3. **失败轨迹和成功轨迹一样重要。** 只有成功案例的实验报告不通过。

4. **当前仓库公开。** 已提交的笔记、实验记录和测试答案都可被阅读；不适合公开的原始草稿留在本地。
   对公开笔记也要诚实标注“不确定”“尚未验证”和 AI 辅助范围，不必为了展示而美化理解。
