...
1. kimi k3 工具调用
   ...
   `search_impl`（54-66 行）——全文件最"反直觉"的设计
   ```python 
   def search_impl(arguments: Dict[str, Any]) -> Any:    return arguments   # 原样返回参数，什么都不做
   ```

   第一次看到会以为是占位符没写完——**其实这就是 Moonshot 内置工具的正确用法**。理解它需要知道 `$web_search` 的协议：当模型请求调用 `$web_search` 时，客户端只需把参数原样回传（285-287 行），**Moonshot 服务端收到 tool 消息后自己执行真正的搜索**，下一轮响应里模型就已经"看到"了搜索结果。搜索动作根本不在你的机器上发生。

   对照 context 项目的 agent.py：那里 `_execute_tool` 在本地跑真实的 Python 函数（汇率换算、PDF 解析），结果由你的代码拼回 messages。而这里是**服务端工具**——docstring 里说的"想换其他模型还想保留搜索能力，就改写这个函数"正是迁移点：换成 DeepSeek 时，你得在这里接真实的搜索 API（如 Tavily/Bing），函数签名不变、循环不用动。
2. kimi k3服务的进一步理解

   **`api.moonshot.cn/v1`**\*\* 不是裸模型推理接口，而是"模型 + 工具运行时"的托管服务层\*\*。拆开看：
   | 层                                                                                                                    | 谁负责           | 干什么                                                                                                                                |
   | -------------------------------------------------------------------------------------------------------------------- | ------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
   | 模型层（K3 权重）                                                                                                           | 推理            | **决定**何时搜、搜什么——生成 `$web_search` 的 tool\_ call token。这个"会用搜索"的能力是 Moonshot 在训练阶段灌进模型的（呼应你之前问的"是不是训练学会的能力"——是，但训练目标是对齐**平台自有的**内置工具） |
   | 平台层（API 网关背后）                                                                                                        | Moonshot 基础设施 | 识别 \`builtin\_function\` 声明 → 收到回传的搜索参数后\*\*在服务端真正执行搜索\*\*（它自己的搜索后端）→ 把结果注入模型上下文 → 模型继续生成                                          |
   | `search_impl` 的"原样返回参数"就是这两层之间的**交接信号**：客户端说"我确认这次调用"，平台拿到后接续执行。所以搜索的流量、IP、结果抓取全部发生在 Moonshot 的机房里，你的机器上一次网络搜索都没发起过。 |               |                                                                                                                                    |
   **这个设计的三个推论**
   1. **工具能力跟着平台走，不跟模型走**。同样是 Kimi 模型：开放平台有 `$web_search`；Kimi Code 平台（`api.kimi.com/coding`）没有；经 OpenRouter 调 kimi 也没有——所以 `_get_tools()` 在兜底时返回空列表。"Kimi Code Key 为什么不行"，根因就在这。
   2. **这是行业通用形态，不止 Moonshot**。OpenAI 的 Responses API 有 hosted tools（`web_search`、`code_interpreter`、`file_search`），Anthropic 有 server tools，字节豆包有联网插件——都是"**API 不止卖推理，还卖托管工具执行**"。Moonshot 官方文档里还有"官方工具 Formula"（把 Python 脚本变成平台侧可触发的工具），属于同一层东西。
   3. **对比客户端工具（context 项目）**：`convert_currency` 这类 `type: "function"` 的工具，平台只做透传，执行在你本地；`$web_search` 这类 `type: "builtin_function"` 的工具，执行在平台侧。**一个 API 请求里两种工具可以混用**，执行边界由 `type` 字段决定。
      总结来看，K3 API = **模型推理（会想）+ 平台工具运行时（会做）**。模型学到的只是"什么时候该搜、搜什么词"，真正的"搜索"这个器官长在平台上。这也是为什么这套代码换个提供商就必须改写 `search_impl`——docstring 里那句"想用其他模型就修改这里的实现"，说的就是"离开 Moonshot 的平台，就得自己长出这个器官"。
