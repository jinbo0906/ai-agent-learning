...
> 基于[《深入理解 AI Agent》第 2 章](https://github.com/bojieli/ai-agent-book/blob/main/book/chapter2.md)整理。文中的实验数字和示例来自原书及配套说明，尚非本仓库独立复测结果。
> [返回章节入口](./README.md) · [先看一页学习笔记](./notes.md)

本页保留完整的阅读梳理与九道思考题讨论稿。思考题答案经 AI 辅助形成，供复盘和质疑；实际运行证据以本仓库 `lab/results/summary.json` 为准，目前尚未提交。
...
![上下文工程为 Agent 提供决策所需信息](image/image_IFgkECSl-K.png)
...
![单轮模型 API 消息结构](image/image_euXR0rf5-T.png)
...
![带工具调用的多轮 Agent 交互](image/image_J1xKvTB_DJ.png)
...
![稳定前缀与动态增长的对话轨迹](image/image_GsudVHSW3B.png)
...
![Chat Template 将结构化消息转换为模型 token](image/image_6PmygfUw3Y.png)
...
![API 消息与 Qwen 模型内部 token 流的对应关系](image/image_PixZ6rrMO3.png)
...
![KV Cache 避免重复计算历史 token](image/image_5UsjYVy8h1.png)
...
![Agent Skills 按需加载专业指令](image/image_Ca-cEhnVAG.png)
...
本页前半部分的代码片段用于理解原书的消息结构与上下文构造，不能当作本仓库已运行的实验。第 2 章尚未提交独立的运行脚本、原始结果或 `lab/results/summary.json`；复现状态和后续入口见[章节首页](./README.md)。
...
2. ★★ Qwen3 的 Chat Template 思维链保留机制只保留 “最后一个真实用户消息之后” 的思考。如果一个 ReAct 循环跨越了上百轮工具调用，累积的思考内容可能消耗大量上下文。你会如何修改这个机制来应对超长循环？DeepSeek R1 曾要求剥离全部历史思考，而 DeepSeek V4 反转为强制回传全部 `reasoning_content`——对比这两种相反的策略，各有什么利弊？这个反转说明了什么？
   ...
   先区分**任务状态**和**模型产生的思维链**。Agent 真正需要长期延续的是目标、已验证事实、排除过的方案和待执行动作；这些应由 Harness 保存为可检查的任务状态。

   对允许客户端控制历史的模型，可以在完整工具回合结束后建立检查点：归档原始轨迹，并用任务状态与必要证据开启下一阶段。不能擅自删改服务商要求回传的思维链块。

   | 策略 | 收益 | 代价 |
   | --- | --- | --- |
   | 本章所述早期 R1：不回传历史思维链 | 输入较短，避免重新喂入模型不期望的旧思考 | 长任务更依赖 Harness 显式保存计划，模型可能重复推导 |
   | DeepSeek 带 `tools` 的思考模式：完整回传历史 `reasoning_content` | 保持工具调用前后的推理连续 | 上下文增长；客户端漏传时 API 会返回错误 |

   [DeepSeek 官方文档](https://api-docs.deepseek.com/zh-cn/guides/thinking_mode/)要求：带 `tools` 的请求须完整回传历史 `reasoning_content`，即使某轮没有实际调用工具；不带 `tools` 时，历史思维链不会拼接进后续上下文。因此，长任务应按所用接口支持的方式，在完整回合之间压缩或续接。

   这说明历史思考的处理方式由模型、Chat Template 和 API 协议共同决定，不能写一套通用的 `strip_reasoning=True`。本章讨论的是特定 Qwen3 模板；切换型号时要重新核对[模板](https://huggingface.co/Qwen/Qwen3.5-4B/blob/main/chat_template.jinja)和[模型说明](https://huggingface.co/Qwen/Qwen3.8-27B)。
...
8. ★★ Skills 机制中，Agent 从 SKILL 文件中动态读取提示词之后，后续的操作能否正确遵从这些指令？不同的模型对 Skills 模式的支持有什么区别？
   ...
   **不能保证。** 要分别观察 Agent 是否发现 Skill、是否读取正文及必要参考文件、后续是否按步骤执行，以及最终产物是否符合要求。仅有一次读取调用不能证明遵从。

   Skills 的目录发现和文件加载由 Agent 运行时提供；模型负责判断和执行。只有聊天 API、没有文件读取或 Skill 激活工具时，磁盘上的 `SKILL.md` 不会自动进入模型上下文。[OpenAI Skills 文档](https://developers.openai.com/api/docs/guides/tools-skills)

   | 能力 | 观察什么 |
   | --- | --- |
   | 选得对 | 同一目录下是否挑中所需 Skill，漏选和误选各有多少 |
   | 读得全 | 是否读取正文要求的参考文件，而不止标题 |
   | 执行得稳 | 多轮工具调用后，是否仍遵守顺序、例外和禁止项 |
   | 会核验 | 是否按 Skill 要求检查最终结果 |

   比较不同模型时，使用同一任务分别运行“无 Skill / 有 Skill”，再比较成功率、调用轨迹和成本。SkillsBench 在其测试范围内观察到收益随任务和模型配置变化，因此不能把“已加载”当成“必然有效”。[SkillsBench 论文](https://www.skillsbench.ai/skillsbench.pdf)
