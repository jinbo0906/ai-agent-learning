...
- [笔记整理](#笔记整理)
  - [上下文：决定 Agent 能力上限的关键](#上下文决定-Agent-能力上限的关键)
  - [Agent 如何调用大模型：理解 API 的上下文结构](#Agent-如何调用大模型理解-API-的上下文结构)
    - [消息的四种角色](#消息的四种角色)
    - [单轮对话：最简单的 API 调用](#单轮对话最简单的-API-调用)
    - [带工具调用的多轮交互：Agent 的核心循环](#带工具调用的多轮交互Agent-的核心循环)
    - [用代码实现 Agent 的核心循环](#用代码实现-Agent-的核心循环)
    - [从 API 视角看上下文的构成](#从-API-视角看上下文的构成)
  - [KV Cache 友好的上下文设计](#KV-Cache-友好的上下文设计)
    - [从 API 消息到模型 Token：Chat Template](#从-API-消息到模型-TokenChat-Template)
    - [KV Cache 的原理与约束](#KV-Cache-的原理与约束)
    - [KV Cache 与 Prompt Cache：两个层级的缓存](#KV-Cache-与-Prompt-Cache两个层级的缓存)
    - [缓存作为架构约束](#缓存作为架构约束)
    - [KV Cache 未必是一次性的：可编辑、可组合的“笔记”](#KV-Cache-未必是一次性的可编辑可组合的笔记)
  - [提示工程：优化系统提示词](#提示工程优化系统提示词)
    - [语气与风格：系统提示词的“人格”](#语气与风格系统提示词的人格)
    - [结构化提示：系统提示词的“格式”](#结构化提示系统提示词的格式)
    - [流程驱动 vs 规则堆砌：系统提示词的“组织方式”](#流程驱动-vs-规则堆砌系统提示词的组织方式)
    - [业务规则细化：系统提示词的“内容”](#业务规则细化系统提示词的内容)
    - [Few-shot 示例：何时给模型看例子](#Few-shot-示例何时给模型看例子)
    - [工具定义的设计](#工具定义的设计)
    - [提示注入：上下文安全的核心威胁](#提示注入上下文安全的核心威胁)
  - [动态提示词与 Agent Skills](#动态提示词与-Agent-Skills)
    - [Skills：领域能力的可组合单元](#Skills领域能力的可组合单元)
    - [如何编写一份可用的 Skill](#如何编写一份可用的-Skill)
    - [Skills 在上下文中的位置](#Skills-在上下文中的位置)
    - [Skills 与工具的关系](#Skills-与工具的关系)
  - [Agent 状态栏：通过元信息增强 Agent 轨迹管理](#Agent-状态栏通过元信息增强-Agent-轨迹管理)
    - [Agent 状态栏的理论基础](#Agent-状态栏的理论基础)
    - [Agent 状态栏的构成](#Agent-状态栏的构成)
    - [Agent 状态栏在上下文中的具体位置](#Agent-状态栏在上下文中的具体位置)
    - [状态更新的两种实现与缓存代价](#状态更新的两种实现与缓存代价)
  - [上下文压缩策略](#上下文压缩策略)
    - [为什么需要压缩：不只是长度问题](#为什么需要压缩不只是长度问题)
    - [上下文学习的内部机制：检索而非推理](#上下文学习的内部机制检索而非推理)
    - [压缩与 KV Cache：看似矛盾，实则互补](#压缩与-KV-Cache看似矛盾实则互补)
    - [生产级的分层压缩机制](#生产级的分层压缩机制)
    - [压缩策略的设计原则](#压缩策略的设计原则)
    - [隔离优于压缩：子 Agent 上下文隔离](#隔离优于压缩子-Agent-上下文隔离)
- [代码学习](#代码学习)
- [思考题](#思考题)
